...
// Конфигурация пула соединений с Connection Pooler
const pool = new Pool({
...
bot.action('my_deals', async (ctx) => {
  try {
    console.log('🔄 Обработка my_deals от пользователя:', ctx.from.id);
...
bot.action('settings', async (ctx) => {
  console.log('🔄 Обработка settings от пользователя:', ctx.from.id);
  await showSettingsMenu(ctx);
});
...
bot.action('balance_menu', async (ctx) => {
  try {
    console.log('🔄 Обработка balance_menu от пользователя:', ctx.from.id);
...
bot.action('deposit', async (ctx) => {
  console.log('🔄 Обработка deposit от пользователя:', ctx.from.id);
...
bot.action('withdraw', async (ctx) => {
  try {
    console.log('🔄 Обработка withdraw от пользователя:', ctx.from.id);
    ...
    // НЕ показываем минимальную сумму на этом этапе
...
bot.action(/contact_seller_(\d+)/, async (ctx) => {
  console.log('🔄 Обработка contact_seller от пользователя:', ctx.from.id);
...
bot.action('main_menu', async (ctx) => {
  console.log('🔄 Обработка main_menu от пользователя:', ctx.from.id);
...
bot.command('ban', async (ctx) => {
  console.log('🔄 Обработка /ban от пользователя:', ctx.from.id);
...
bot.command('deals', async (ctx) => {
  console.log('🔄 Обработка /deals от пользователя:', ctx.from.id);
...
bot.command('cherryteam', async (ctx) => {
  console.log('🔄 Обработка /cherryteam от пользователя:', ctx.from.id);
...
bot.command('stats', async (ctx) => {
  console.log('🔄 Обработка /stats от пользователя:', ctx.from.id);
...
bot.on('web_app_data', async (ctx) => {
  try {
    console.log('🔄 Обработка web_app_data от пользователя:', ctx.from.id);
...
// Обработка текстовых сообщений (ИСПРАВЛЕННЫЙ ОБРАБОТЧИК)
bot.on('text', async (ctx) => {
  try {
    const userId = ctx.from.id;
    const message = ctx.message.text;
    
    console.log('📨 Получено сообщение от пользователя:', userId, 'Текст:', message);
    
    // Обработка команд - пропускаем их для обработчиков bot.command()
    if (message.startsWith('/')) {
      const command = message.split(' ')[0];
      console.log('🔍 Обнаружена команда:', command);
      
      // Команды /ban, /deals, /stats, /cherryteam обрабатываются автоматически
      if (['/ban', '/deals', '/stats', '/cherryteam'].includes(command)) {
        console.log('✅ Команда передана обработчику команд');
        return; // Пропускаем для обработки bot.command()
      }
    }
...
async function processWithdrawalRequest(ctx, message, userState) {
  ...
  try {
    ...
    // ТЕПЕРЬ показываем минимальную сумму только после ввода
