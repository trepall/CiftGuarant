...
// Фото константы
const SETTINGS_PHOTO = 'https://i.ibb.co/6SMsH2d/image.jpg';
const MAIN_MENU_PHOTO = 'https://i.ibb.co/6SMsH2d/image.jpg';
const DEALS_PHOTO = 'https://i.ibb.co/DHHpbKPN/image.jpg';
...
async function safeSendPhoto(ctx, photoUrl, caption, keyboard) {
  try {
    ...
    console.error('❌ Ошибка фото, отправляем текст:', photoError.message);
    // Фолбэк на текст
...
async function showMainMenu(ctx) {
  try {
    ...
    await safeSendPhoto(ctx, MAIN_MENU_PHOTO, caption, keyboard);
...
async function showSettingsMenu(ctx) {
  try {
    ...
    await safeSendPhoto(ctx, SETTINGS_PHOTO, caption, keyboard);
...
bot.action('my_deals', async (ctx) => {
  try {
    ...
    if (deals.rows.length === 0) {
      ...
      await safeSendPhoto(ctx, DEALS_PHOTO, '📭 У вас пока нет сделок', keyboard);
      return;
    }
    ...
    await safeSendPhoto(ctx, DEALS_PHOTO, dealsText, keyboard);
...
async function startBot() {
  try {
    ...
    if (error.response?.error_code === 409) {
      console.log('⚠️  Другой экземпляр бота уже запущен. Завершаем...');
      process.exit(0);
    }
