...
// Обработка стартовой команды со сделкой
bot.start(async (ctx) => {
  try {
    const startPayload = ctx.startPayload;
    
    if (startPayload && startPayload.startsWith('deal_')) {
      // Покупатель перешел по ссылке сделки
      ...
      if (deal.seller_id === ctx.from.id) {
        await ctx.reply('ℹ️ Это ваша собственная сделка');
        return await showMainMenu(ctx);
      }
      
      // Показываем меню оплаты для покупателя
      const caption = `🛍️ *Сделка #${deal.deal_id}*\n\n` +
        `🎯 Тип: ${getDealTypeText(deal.deal_type)}\n` +
        `💰 Сумма: ${deal.amount} ${deal.currency}\n` +
        `📝 Описание: ${deal.product_info}\n\n` +
      ...
      const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('💎 Оплатить TON', `pay_ton_${dealId}`)],
        [Markup.button.callback('💵 Оплатить USDT', `pay_usdt_${dealId}`)],
        [Markup.button.callback('⏪ Главное меню', 'mainMenu')]
      ]);
