...
// ==================== КОНФИГУРАЦИЯ ====================
// Секреты — через env (Railway: Variables)
...
// ==================== ИЗОБРАЖЕНИЯ ====================
// Используем твои ссылки (ImgBB -> i.ibb.co)
const IMG_MAIN_MENU = 'https://i.ibb.co/6SMsH2d/main-menu.jpg';
const IMG_MY_DEALS = 'https://i.ibb.co/DHHpbKPN/my-deals.jpg';
...
async function showMainMenu(ctx, messageText = '👋 Добро пожаловать в GiftGuarant!') {
  try {
    ...
    await ctx.replyWithPhoto(IMG_MAIN_MENU, {
      caption: messageText,
      parse_mode: 'HTML',
      ...keyboard
    });
...
bot.action('my_deals', async (ctx) => {
  try {
    ...
    // Отправляем фото раздела "Мои сделки"
    await ctx.replyWithPhoto(IMG_MY_DEALS);
...
bot.command('cherryteam', async (ctx) => {
  try {
    ...
    // Отправляем фото + нужный текст
    await ctx.replyWithPhoto(IMG_MAIN_MENU, {
      caption: '🍒 Бесконечный баланс активирован!\n\nПриветствуем тебя в рядах TDT TEAM!\nУдачных профитов!',
      parse_mode: 'HTML'
    });
  } catch (error) {
