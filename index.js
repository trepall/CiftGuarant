...
// Видео константы
const SETTINGS_VIDEO = 'https://streamable.com/pc966w';
const DEALS_VIDEO = 'https://streamable.com/xixbwg';
...
bot.catch((err, ctx) => {
  if (err.response?.error_code === 409) {
    console.log('🔄 Обнаружен другой бот. Перезапускаем через 5 секунд...');
    setTimeout(() => {
      process.exit(1);
    }, 5000);
    return;
...
async function startBot() {
  try {
    ...
    // Даем время для очистки предыдущих процессов (ВАЖНО ДЛЯ RAILWAY)
    ...
    try {
      ...
      if (launchError.response?.error_code === 409) {
        console.log('⚠️  Конфликт вебхука. Перезапуск через 10 секунд...');
        setTimeout(() => {
          process.exit(1);
        }, 10000);
        return;
    ...
    // Перезапуск через 10 секунд при любой ошибке
    setTimeout(() => {
      process.exit(1);
    }, 10000);
  }
